 ...
 */
//@Controller
public class Task0TimerController {
	...
	public void execute() {	
		StringBuilder title = new StringBuilder("【续代预警】拖地车代理到期提醒");
		StringBuilder header = new StringBuilder("<h1>以下拖地车代理合同即将到期，请及时处理!</h1>\n\r"
		...
		condition.getContract().setContractType("代理商");//设置合同类型
		...
		if (contractDetails != null && !contractDetails.isEmpty()) {
			for(ContractDetail contractDetail : contractDetails) {
				content.append("<tr>"
						+ "<td style='border-bottom:1px solid #ccc;'>"+ contractDetail.getContract().getCustomer().getCustomerName() +"</td>"
