 ...
 */
//@Controller
public class Task1TimerController {
	...
	//@Scheduled(cron = "0/10 * * * * ?")
	public void execute() {		
		StringBuilder title = new StringBuilder("【回款预警】租赁车辆分期到期提醒");
		StringBuilder header = new StringBuilder("<h1>以下租赁车辆分期即将到期，请及时处理!</h1>\n\r"
		...
		condition.getContractDetail().getContract().setContractType("租赁");// 租赁合同
		...
		if (stages != null && !stages.isEmpty()) {
			for(Stage stage : stages) {
				content.append("<tr>"
						 + "<td style='border-bottom:1px solid #ccc;'>"+ stage.getContractDetail().getProjectAddress() +"</td>"
