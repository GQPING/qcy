...
<script type="text/html" id="contractTypeTpl">
	<span class="label label-primary radius" data-d="{{d.contractType}}" onmouseover="show_shopm(this)" onmouseout="exit_shopm()">{{ d.contractType }}</span>
</script>
<!--合同状态-->
			...
			table.on('toolbar(LAY_table_user)', function (obj) {
				...
				switch (obj.event) {
					case 'create':
						...
						tk('添加合同', ['100%', '100%'], 'userbj', $("#bjnr"), ['确定', '关闭'], _funbtnxs);
						// 下拉框值改变动态设置其它输入
					...
					case 'update':
						...
						tk('修改合同', ['100%', '100%'], 'userbj', $("#bjnr"), ['确定', '关闭'], _funbtnxs);
						// 下拉框值改变动态设置其它输入
						form.on('select(marketPerson)', function (data) {
							if(data.value == "其它__") {
								$("#marketPersonInput").val('');
					...
					case 'detail':
						...
						var _funbtnxs = {
							yesxs1: function (index, layero) {
								...
								$("#payRemindDays").val('15');
								$("#payRemindDaysInput").val('15');
						...
						tk('<span style="font-weight: bold">【代理合同】</span>添加合同明细', ['100%', '100%'], 'userbj', $("#bjnr2"), ['确定', '关闭'], _funbtnxs);
