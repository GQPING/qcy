...
<div>
	<nav class="breadcrumb">
		...
		<span class="c-gray en">&gt;</span> 代理合同
...
<script type="text/html" id="toolbarDemo">
	<div class="layui-btn-container">
		...
		<a id="carsInfo" href="javascript:;" style="color: white" class="layui-table-link">
		    <button id="carsinfobtn" style="display: none;" class="layui-btn layui-btn-sm" lay-event="carsinfo">
			    <i class="Hui-iconfont">&#xe669;</i>车辆信息
		    </button>
		</a>
		<a id="stepInfo" href="javascript:;" style="color: white" class="layui-table-link">
			<button id="stepinfobtn" style="display: none;" class="layui-btn layui-btn-sm" lay-event="stepinfo">
				<i class="Hui-iconfont">&#xe6f3;</i> 分期信息
			</button>
		</a>
...
<!--代理日期-->
<script type="text/html" id="dateStageTpl">
	{{# var fn = function(){
...
<!--日期周期-->
<script type="text/html" id="dateCycleTpl">
	{{# var fn = function(){
...
<!--日期状态-->
<script type="text/html" id="dateStateTpl">
	{{# var fn = function(){
...
<script type="text/javascript">
	...
	function CanPayChange(){
		var canPayQuota = $('#canPayQuota').val() == '' ? 0.0 : parseFloat($('#canPayQuota').val());// 应收变化
		var productDiscount = $('#productDiscount').val() == '' ? 0.0 : parseFloat($('#productDiscount').val());// 坏账
		...
		layui.use(['form', 'table', 'laydate'], function () {
			...
			let dateEntryStart2 = laydate.render({
				...
				trigger: 'click',
					...
					let finalDate = $("#finalDate").val();//结束日期
					if(startDate!='' && finalDate!=''){
						...
						if(minusDays < 0){
							$("#startDate").val(times);
							minusDays = $("#dateCycle").val();
							layer.msg('开始日期大于结束日期!', {icon: 0, time: 1000, anim: 6});
							form.render('select');
						}
						//开始日期变化 -> 周期变化
						$("#dateCycle").val(minusDays);//周期变化
					} else{
						document.getElementById("dateCycle").value = 0;
					}
				}
			});
			let dateEntryEnd2 = laydate.render({
				...
				trigger: 'click',//  触发方式
					...
					let startDate = $("#startDate").val();//开始日期
			...
			table.render({
				...
				, id: 'LAY_table_user'
				, url: '${pageContext.request.contextPath }/contractDetail/searchList/代理商' //数据请求路径
				, method: 'post'
			...
			table.on('radio(LAY_table_user)', function(obj){
				$('#carsInfo').attr('onclick','Hui_admin_tab(this)');
				$('#carsInfo').attr('data-href','${ctx}/vehicleDetail/detail/'+obj.data.id+'/代理');
				$('#carsInfo').attr('data-title','【'+obj.data.contract.customer.customerName+'】车辆信息');

				$('#stepInfo').attr('onclick','Hui_admin_tab(this)');
				$('#stepInfo').attr('data-href','${ctx}/stage/detail/'+obj.data.id+'/代理');
				$('#stepInfo').attr('data-title','【'+obj.data.contract.customer.customerName+'】分期信息');

				$('#cashInfo').attr('onclick','Hui_admin_tab(this)');
				$('#cashInfo').attr('data-href','${ctx}/cash/detail/'+obj.data.id+'/代理');
				$('#cashInfo').attr('data-title','【'+obj.data.contract.customer.customerName+'】收款信息');

				$('#billInfo').attr('onclick','Hui_admin_tab(this)');
				$('#billInfo').attr('data-href','${ctx}/bill/detail/'+obj.data.id+'/代理');
				$('#billInfo').attr('data-title','【'+obj.data.contract.customer.customerName+'】开票信息');
			});
			// 头工具栏事件
					...
					case 'update':
						...
						var _funbtnxs = {
							yesxs1: function (index, layero) {
								...
								var canPayQuota= $("#canPayQuota").val().trim();;//应收金额
								if(canPayQuota == ""){
									layer.msg('请输入应收金额', {icon: 0, time: 1000, anim: 6});
									return false;
								}
								var productDiscount = $("#productDiscount").val().trim();//押金
								if (productDiscount == "") {
								...
								if (productDeposit == "") {
									layer.msg('请输入代理押金', {icon: 0, time: 1000, anim: 6});
									return false;
								}
								...
								var payRemindDays = $("#payRemindDaysInput").val().trim();
								if (payRemindDays == "") {
									layer.msg('请设置到期提醒', {icon: 0, time: 1000, anim: 6});
									return false;
								}
								...
								var sdate1 = startDateStr.split('-');
								var startDate = new Date(sdate1[0], sdate1[1]-1, sdate1[2]);//时分秒设置为0
								var finalDateStr = $("#finalDate").val().trim();
								if (finalDateStr == "") {
									layer.msg('请选择结束日期', {icon: 0, time: 1000, anim: 6});
									return false;
								}
								var sdate2 = finalDateStr.split('-');
								var finalDate = new Date(sdate2[0], sdate2[1]-1, sdate2[2]);//时分秒设置为0
								var dateCycle = $("#dateCycle").val().trim();
								var unPayQuota = $("#unPayQuota").val().trim();//未收金额
								var payState = $("#payState").val().trim();//付款状态
								...
								$("#payRemindDays").val(dataRow[0].payRemindDays);
								$("#payRemindDaysInput").val(dataRow[0].payRemindDays);
								if($("#payRemindDays").val()==null || $("#payRemindDays").val()==""){
									$("#payRemindDays").val("其它__");
									document.getElementById("payRemindDaysInput").style.display="";
								}else{
									document.getElementById("payRemindDaysInput").style.display="none";
								}
								$("#startDate").val(layui.util.toDateString(dataRow[0].startDate, 'yyyy-MM-dd'));
								times=layui.util.toDateString(dataRow[0].startDate, 'yyyy-MM-dd');
								timef=layui.util.toDateString(dataRow[0].finalDate, 'yyyy-MM-dd');
								$("#finalDate").val(layui.util.toDateString(dataRow[0].finalDate, 'yyyy-MM-dd'));
								$("#dateCycle").val(dataRow[0].dateCycle);
						...
						tk('<span style="font-weight: bold">【代理合同】</span>修改合同明细', ['100%', '100%'], 'userbj', $("#bjnr"), ['确定', '关闭'], _funbtnxs);
						...
						form.on('select(payRemindDays)', function (data) {
							if(data.value == "其它__") {
								$("#payRemindDaysInput").val('');
								document.getElementById("payRemindDaysInput").style.display="";
							} else{
								$("#payRemindDaysInput").val(data.value);
								document.getElementById("payRemindDaysInput").style.display="none";
							}
							form.render('select');
						});
			...
			var $ = layui.$, active = {
				reload: function () {
					...
					table.reload('LAY_table_user', {
						url: '${pageContext.request.contextPath }/contractDetail/searchList/代理商',
						method: 'post',
						contentType: "application/json;charset=UTF-8",
						page: {
							curr: 1 //重新从第 1 页开始
	...
	function permission(){
		...
		$.ajax({
			...
			method: 'post',
			data: JSON.stringify({id:'${sessionScope.CurrentUser.roleID}',menuNameString:'车辆信息',operationNameString:'查看'}),
			contentType: "application/json;charset=UTF-8",
			dataType: "json",
			success: function (data) {
				document.getElementById("carsinfobtn").style.display = "";
			}, error: function (data) {
				//layer.msg('权限获取异常!', {icon: 5, time: 1000, anim: 6});
			}
		});
		$.ajax({
			url: '${pageContext.request.contextPath }/rolePermission/search',
			method: 'post',
			data: JSON.stringify({id:'${sessionScope.CurrentUser.roleID}',menuNameString:'分期信息',operationNameString:'查看'}),
			contentType: "application/json;charset=UTF-8",
			dataType: "json",
			success: function (data) {
				document.getElementById("stepinfobtn").style.display = "";
			}, error: function (data) {
				//layer.msg('权限获取异常!', {icon: 5, time: 1000, anim: 6});
			}
		});
	...
	<form class="layui-form">
		<table class="layui-table layui-xs" lay-skin="nob">
			...
			<tr>
				...
				<td align="left">
					<input type="text" id="canPayQuota" value="0" oninput="CanPayChange()" autocomplete="off" class="layui-input"
			...
			<tr>
				...
				<td align="left">
					<input type="text" id="productDiscount" value="0" oninput="CanPayChange()" autocomplete="off" class="layui-input"
						   onblur="if(value=='')execCommand('undo')" Onkeyup="if(isNaN(value))execCommand('undo')" Onpaste="return false;" placeholder="请输入">
				</td>
			</tr>
			<tr>
				<td align="right">代理押金：<span style="margin-right: 22px">（元）</span></td>
				<td align="left">
			...
			<tr>
				...
				<td align="left">
					<select class="layui-input" id="payRemindDays" placeholder="请选择" lay-filter="payRemindDays">
						<option value="">请选择</option>
						<option value="7">7天内到期提醒</option>
						<option value="15">15天内到期提醒</option>
						<option value="30">30天内到期提醒</option>
						<option value="45">45天内到期提醒</option>
						<option value="其它__">其它__</option>
					</select>
					<input type="text" style="display: none" id="payRemindDaysInput" autocomplete="off" onblur="if(value=='')execCommand('undo')"
			...
			<tr>
				...
				<td align="left">
					<input type="text" id="startDate" autocomplete="off" class="layui-input" placeholder="请选择">
				</td>
			</tr>
			<tr>
				<td align="right">结束日期：</td>
				<td align="left">
					<input type="text" id="finalDate" autocomplete="off" class="layui-input" placeholder="请选择">
				</td>
			</tr>
			<tr>
				<td align="right">周 期 /月：</td>
				<td align="left">
					<input type="text" id="dateCycle" value="0" class="layui-input layui-btn-disabled" disabled>
				</td>
			</tr>
			<tr>
