...
<div>
    <nav class="breadcrumb">
        ...
        <span class="c-gray en">&gt;</span> 租赁合同
...
<script type="text/javascript">
    ...
    function PriceChange(){
        ...
        var dateCycle = $('#dateCycle').val() == '' ? 0.0 : parseFloat($('#dateCycle').val());//周期
        var total = productPrice * dateCycle;// 应收 = 单价 * 周期
    ...
    $(function () {
        ...
        layui.use(['form', 'table', 'laydate'], function () {
            ...
            let dateEntryStart2 = laydate.render({
                ...
                trigger: 'click',
                    ...
                    if(startDate!='' && finalDate!=''){
                        ...
                        //开始日期变化 -> 周期变化、应收变化、未收变化、状态变化
                        $("#dateCycle").val(minusDays);//周期变化
                        var productPrice = $('#productPrice').val() == '' ? 0.0 : parseFloat($('#productPrice').val());// 单价
                        var total = productPrice * minusDays;// 应收 = 单价 * 周期
                        total = total.toFixed(2);//保留两位小数
                        $("#canPayQuota").val(total);// 应收变化
                        var hasPayQuota = $('#hasPayQuota').val() == '' ? 0.0 : parseFloat($('#hasPayQuota').val());// 已收
                        var productDiscount = $('#productDiscount').val() == '' ? 0.0 : parseFloat($('#productDiscount').val());// 优惠
            ...
            table.render({
                ...
                , id: 'LAY_table_user'
                , url: '${pageContext.request.contextPath }/contractDetail/searchList/租赁' //数据请求路径
                , method: 'post'
            ...
            table.on('radio(LAY_table_user)', function(obj){
                ...
                $('#stepInfo').attr('data-href','${ctx}/stage/detail/'+obj.data.id+'/租赁');
                $('#stepInfo').attr('data-title','【'+obj.data.projectAddress+'】分期信息');

                $('#cashInfo').attr('onclick','Hui_admin_tab(this)');
                $('#cashInfo').attr('data-href','${ctx}/cash/detail/'+obj.data.id+'/租赁');
                $('#cashInfo').attr('data-title','【'+obj.data.projectAddress+'】收款信息');

                $('#billInfo').attr('onclick','Hui_admin_tab(this)');
                $('#billInfo').attr('data-href','${ctx}/bill/detail/'+obj.data.id+'/租赁');
            ...
            table.on('toolbar(LAY_table_user)', function (obj) {
                ...
                switch (obj.event) {
                    ...
                    case 'update':
                        ...
                        tk('<span style="font-weight: bold">【租赁合同】</span>修改合同明细', ['100%', '100%'], 'userbj', $("#bjnr"), ['确定', '关闭'], _funbtnxs);
                        // 下拉框值改变动态设置其它输入
            ...
            var $ = layui.$, active = {
                reload: function () {
                    ...
                    table.reload('LAY_table_user', {
                        url: '${pageContext.request.contextPath }/contractDetail/searchList/租赁',
                        method: 'post',
                        contentType: "application/json;charset=UTF-8",
                        page: {
                            curr: 1 //重新从第 1 页开始
...
<div id="bjnr" style="display: none;">
    <form class="layui-form" id="form-validate">
        <table class="layui-table layui-xs" lay-skin="nob">
            ...
            <tr>
                <td align="right">产品单价：<span style="margin-right: 0px;">（元/月/台）</span></td>
                <td align="left">
