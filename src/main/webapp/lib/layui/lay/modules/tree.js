        ...
        layui.each(r, function (a, r) {
            var l = r.children && r.children.length > 0,
                ...
                h = i(['<div data-id="' + r.id + '" class="layui-tree-set' + (r.spread ? " layui-tree-spread" : "") + (r.checked ? " layui-tree-checkedFirst" : "") + '">', '<div class="layui-tree-entry">', '<div class="layui-tree-main">', function () {
                    ...
                    return t.isJump && r.href ? '<a onmouseover = "show_shopm2(this)" onmouseout = "exit_shopm2()" data-d="' + (r.title || r.label || t.text.defaultNodeName) + '" href="' + r.href + '" target="_blank" class="' + y + '">' + (r.title || r.label || t.text.defaultNodeName) + "</a>" : '<span onmouseover = "show_shopm2(this)" onmouseout = "exit_shopm2()" data-d="' + (r.title || r.label || t.text.defaultNodeName) + '" class="' + y + (r.disabled ? " " + d : "") + '">' + (r.title || r.label || t.text.defaultNodeName) + "</span>"
                }(), "</div>", function () {
                    if (!t.edit) return "";
                    var e = {
...
// 显示提示
function show_shopm2(t) {
    var row = $(t).attr('data-d'); //获取显示内容
    if (row != null && row != "") {
        //小tips
        tips2 = layer.tips("<span style='color:#000;'>" + row + "</span>", t, {
            tips: [1, '#F5DEB3'],
            time: 2000,
            area: 'auto'
        })
